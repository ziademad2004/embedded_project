    ...
    for (int i = 0; i < 3; i++) {
        for (int j = 0; j < 3; j++) {
            if (gameLogic->getCell(i, j) == GameLogic::Player::None) {
                ...
                if (gameLogic->checkGameState() == GameLogic::GameState::OWins) {
                    // Undo the move (reset the game and replay moves except the last one)
                    auto moves = gameLogic->getMoveHistory();
                    moves.pop_back(); // Remove the test move
    ...
    for (int i = 0; i < 3; i++) {
        for (int j = 0; j < 3; j++) {
            if (gameLogic->getCell(i, j) == GameLogic::Player::None) {
                ...
                if (gameLogic->checkGameState() == GameLogic::GameState::XWins) {
                    // Undo the move
                    auto moves = gameLogic->getMoveHistory();
                    moves.pop_back(); // Remove the test move
