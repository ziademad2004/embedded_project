...
GameHistory& GameHistory::getInstance()
{
    ...
    // Check for and emit any errors from const methods
    instance->checkForErrors();
...
void GameHistory::checkForErrors()
{
    if (!pendingErrorMessage.isEmpty()) {
        // Check if we have cache updates to process
        if (pendingErrorMessage.contains("CACHE_UPDATE:") || pendingErrorMessage.contains("CACHE_ADD:")) {
            QStringList messages = pendingErrorMessage.split('\n', Qt::SkipEmptyParts);

            for (const QString &message : messages) {
                if (message.startsWith("CACHE_ADD:")) {
                    // Extract game ID and add it to cache
                    int gameId = message.mid(10).toInt();

                    // Fetch the game details again (this time non-const call is fine)
                ...
                else if (!message.startsWith("CACHE_UPDATE:") && !message.startsWith("CACHE_ADD:")) {
                    // Real error message to emit
                    emit gameHistoryError(message);
                }
            }
        } else {
            // Just a regular error
            emit gameHistoryError(pendingErrorMessage);
        }
...
QVector<GameHistory::GameRecord> GameHistory::getUserGameHistory(int userId) const
{
    ...
    if (userGames.size() < 20) {
        ...
        // Create a temporary vector for new records to add to the cache
        QVector<GameRecord> newCacheRecords;
        ...
        for (const auto &gameId : gameIds) {
            ...
            if (!alreadyHave) {
                ...
                if (query.exec() && query.next()) {
                    ...
                    // Add to temporary cache vector
                    if (gameRecords.size() < 100) {
                        newCacheRecords.append(record);
                    }
                }
            }
        ...
        // Store the new records for later addition to the cache
        if (!newCacheRecords.isEmpty()) {
            pendingErrorMessage += "CACHE_UPDATE:" + QString::number(newCacheRecords.size());
        }
    }
...
GameHistory::GameRecord GameHistory::getGameDetails(int gameId) const
{
    ...
    if (query.exec() && query.next()) {
        ...
        // Flag this record for addition to the cache
        pendingErrorMessage += "CACHE_ADD:" + QString::number(record.gameId);
    } else {
        // Store error for later emission since we're in a const method
        pendingErrorMessage = "Failed to retrieve game details";
    }
